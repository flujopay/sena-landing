...
export const useModalStore = create<ModalState>((set, _) => ({
  ...
  showModal: (modal) =>
    set((state) => {
      const modalType = (modal.content as any)?.type?.name || (modal.content as any)?.type?.displayName
      if (modalType) {
        const existingModalOfSameType = state.modals.find((m) => {
          const existingType = (m.content as any)?.type?.name || (m.content as any)?.type?.displayName
