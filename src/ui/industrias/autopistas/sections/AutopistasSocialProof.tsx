export const AutopistasSocialProof = () => {
  const clientTypes = [
    'Concesionarias viales',
    'Operadores de autopistas urbanas e interurbanas',
    'Infraestructura de peajes',
    'Administradores de TAG',
    'Operadores de servicios de tránsito',
  ]
  const metrics = [
    { value: '35%', label: 'Reducción en reclamos de facturación' },
    { value: '-25 días', label: 'Mejora en el DSO promedio' },
    { value: '99%', label: 'Visibilidad sobre cartera vigente de flotas críticas' },
  ]
      ...
      <div className="max-w-[1280px] mx-auto px-4 md:px-10">
        ...
        <div className="flex flex-wrap gap-2 md:gap-3 mb-12">
          {clientTypes.map((t) => (
            <span
              key={t}
              className="bg-white text-brand-primary-dark text-xs md:text-sm font-semibold px-4 py-2 rounded-full border border-brand-primary/10"
            >
              {t}
            </span>
          ))}
          ...
          <div className="flex items-start gap-4">
            ...
            <div>
              <p className="text-slate-700 text-base md:text-lg leading-relaxed italic">
                Gestionábamos contratos B2B con grandes flotas y altos niveles de disputa por respaldo de
                tránsitos. Con Sena ordenamos la cobranza y recuperamos el 35% de la cartera vencida sin
                afectar la relación con nuestros clientes empresa.
              </p>
              <div className="mt-4">
                <p className="text-brand-primary-dark font-bold text-sm">Gerente de Finanzas</p>
                <p className="text-slate-500 text-sm">Concesionaria Vial</p>
              </div>
            </div>
          </div>
        </div>
